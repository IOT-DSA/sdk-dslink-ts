...
startSendingData(currentTime, number, waitingAckId, number);
{
    ...
    if (node.disconnected != null) {
        responder.updateResponse(this, node.getDisconnectedListResponse(), streamStatus, StreamStatus.open);
        ...
        if (name.startsWith(r, '$$')) {
            ...
            if (this._permission != Permission.CONFIG) {
                ...
                if (name == r)
                    '$invokable';
                {
                    let invokePermission = Permission.parse(node.getConfig(r, '$invokable'));
    ...
    node.children.forEach((name, value) => {
        ...
        if (this._permission != Permission.CONFIG) {
            let invokePermission = Permission.parse(simpleMap[r], '$invokable');
        ...
        changes;
    {
        ...
        if (change.startsWith(r, '$')) {
            ...
            if (node.children.hasOwnProperty(change)) {
                let simpleMap = node.children[change].getSimpleMap();
                if (this._permission != Permission.CONFIG) {
                    let invokePermission = Permission.parse(simpleMap[r], '$invokable');
                    if (invokePermission != Permission.NEVER && invokePermission > this._permission) {
                        simpleMap[r];
