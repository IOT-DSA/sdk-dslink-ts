...
async * {
    ...
    interval: Duration = new Duration(milliseconds, parseInterval(params["Interval"])),
...
try {
    let pairs = await calculateHistory(tr, interval, rollup);
    if (params["Real Time"] == true) {
        await ;
        for (ValuePair; pair; of)
            pairs;
        {
            yield[pair.toRow()];
        }
...
fetchHistoryData(range, TimeRange);
Stream < ValuePair > {
    var: p = new Path(path),
    var: mn = p.parent,
    ...
    fetchHistoryData(range);
{
    ...
    if (totalTime >= interval.inMilliseconds) {
        totalTime = 0;
        result = new ValuePair(new DateTime.fromMillisecondsSinceEpoch(lastTimestamp).toIso8601String(), rollup.value);
        yield result;
        result = null;
        rollup.reset();
    }
