...
PrivateKey;
{
    if (str.contains(" ")) {
        ...
        let pri = new ECPrivateKey(d, this._secp256r1);
        var Q = this._secp256r1.curve.decodePoint(Base64.decode(ss[1]));
        let pub = new ECPublicKey(Q, this._secp256r1);
...
{
    ECPoint;
    Q = this._secp256r1.curve.decodePoint(bytes);
...
Promise < ECDHImpl > getSecret(key, string);
async;
{
    ECPoint;
    p = ecPrivateKey.parameters.curve.decodePoint(Base64.decode(key));
...
DSRandomImpl([seed, number = -1]);
{
    ...
    final;
    key = new KeyParameter(new Uint8Array.fromList(keyBytes));
    r = new Math.Random((new DateTime.now()).millisecondsSinceEpoch);
    final;
    iv = new Uint8Array.fromList([
        r.nextInt(256),
        r.nextInt(256),
        r.nextInt(256),
        r.nextInt(256),
...
addEntropy(str, string);
{
    ...
    final;
    bytes = new Uint8Array.fromList(utf);
    final;
...
bigintToUint8Array(input, BigInteger);
Uint8Array;
{
